// Vidy / Wecollege decryption algorithm (keystream generator & XOR cipher)
...
export function decryptVidyPayload<T = unknown>(ciphertext: string, seed: string, tmdbId: number): T {
  ...
  for (let n = 0; n < MAGIC_PREFIX.length; n++) {
    if (cipherBytes[n] !== MAGIC_PREFIX[n]) {
      throw new Error("Vidy decrypt failed: bad seed or payload header mismatch");
    }
  }

  const rawBytes = cipherBytes.subarray(MAGIC_PREFIX.length);
  const jsonStr =
