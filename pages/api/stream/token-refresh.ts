...
export default async function handler(
  ...
  const perUserKey = `rl:token-refresh:user:${sessionId}`;
  const perIpKey = `rl:token-refresh:ip:${ipPrefix}`;
  ...
  if (!userLimit.allowed) {
    return res.status(429).json({ error: "Too many token refresh requests" });
  }
  ...
  if (!ipLimit.allowed) {
    return res.status(429).json({ error: "Too many token refresh requests from IP" });
  }
